...
io.on('connection', (socket) => {
    socket.on('chat message 1', (msg) => {
        fs.appendFileSync(__dirname + '/chat/db1.txt', `<script>a(\'${msg}\');</script>`); // Puts Message In File
    ...
    socket.on('chat message 2', (msg) => {
      fs.appendFileSync(__dirname + '/chat/db2.txt', `<script>a(\'${msg}\');</script>`); // Puts Message In File
